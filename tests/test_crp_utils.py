...
def test_compute_features_weights_per_client_uniform_mode_skips_crp_and_logits(
    monkeypatch,
):
    calls = {"logits": 0, "crp": 0, "load": 0}
    ...
    assert calls["crp"] == 0
    assert calls["load"] == 0
    assert result.shape == torch.Size([2, n_clients, 2])  # celeba has 2 classes
    assert torch.allclose(result, torch.full_like(result, 1.0 / n_clients))
