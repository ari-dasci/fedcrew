...
def train(client_flex_model: FlexModel, client_data: Dataset, l1_factor=args.l1):
    ...
    for _ in range(EPOCHS):
        for images, labels in client_dataloader:
            ...
            images_a = strong_transform(images)
            images_b = weak_transform(images)
            optimizer.zero_grad()
            pred, vector_a = wrapper(images_a)
            loss = criterion(pred, labels)
            pred, vector_b = wrapper(images_b)
            loss += criterion(pred, labels)
            if l1_factor > 0.0:
            ...
            cos_dist = (
                1 - torch.nn.functional.cosine_similarity(vector_a, vector_b, dim=1)
            ).sum()
            loss += 0.5 * cos_dist
            loss.backward()
            optimizer.step()
...
def compute_features_weights_per_client(
    ...
    for label in labels:
        ...
        label_probs = torch.where(
            label_probs > alpha, label_probs, float("-inf")
        )  # softmax(-inf) = 0
