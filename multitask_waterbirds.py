...
from models import get_transforms, AutoencoderMultitask
...
round_number = 0
parser = argparse.ArgumentParser(description="Multitaask brrrr")
parser.add_argument(
...
parser.add_argument("--dataset", type=str, default="waterbirds_multi")
...
class WrapperForAutoencoder(nn.Module):
    def __init__(self, autoencoder):
        super(WrapperForAutoencoder, self).__init__()
        self.autoencoder = autoencoder

    def forward(self, x):
        return self.autoencoder(x)[1]
...
def train(client_flex_model: FlexModel, client_data: Dataset):
    ...
    assert isinstance(model, AutoencoderMultitask)
...
def client_crp(client_model: nn.Module, client_id: int, sample_dataset: Dataset):
    ...
    for sample_id in range(len(data)):
        ...
        heatmap, _ = extract_heatmap(
            WrapperForAutoencoder(client_model),
            layer="autoencoder.encoder.12",
...
def get_crp_attribution(model: nn.Module, sample_dataset: Dataset, layer: str):
    ...
    for sample, label in dataloader:
        ...
        rel_c = cc.attribute(
            attr.relevances[layer].view(attr.relevances[layer].shape[0], -1),
            abs_norm=True,
        )
        rel_c = rel_c.flatten()
...
def compute_features_weights_per_client(
    ...
    for client_id in range(len(weights)):
        ...
        clients_probs[client_id] = group_correct_class_probs(
            WrapperForAutoencoder(client_model), subsamples
        )  # Dict with id -> (Dict with label -> [prob])
        clients_relevances[client_id] = get_crp_attribution(
            WrapperForAutoencoder(client_model), subsamples, "autoencoder.encoder.12"
...
def train_base(pool: FlexPool, n_rounds=100):
    ...
    for i in tqdm(range(n_rounds)):
        ...
        loss, acc, confusion_matrix, rec_loss = pool.servers.map(obtain_metrics)[0]
        if writer:
            writer.add_scalar("Loss", loss, round_number)
            writer.add_scalar("Accuracy", acc, round_number)
            writer.add_scalar("Reconstruction Loss", rec_loss, round_number)
            fig = plt.figure()
            plt.imshow(confusion_matrix)
            plt.show()
            writer.add_figure("confusion_matrix", fig, round_number)
        print(
            f"ROUND {round_number}: loss {loss:7}, acc {acc:7}, rec_loss {rec_loss:7}"
        )
